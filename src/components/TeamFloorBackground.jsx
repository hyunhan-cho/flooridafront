// src/components/TeamFloorBackground.jsx
...
export default function TeamFloorBackground({ personalLevel, animate = true }) {
  ...
  const lvl = Number(personalLevel);

  // ✅ transition은 부모가 준 animate로만 결정
  const transition = animate ? "bottom 0.5s ease-in-out 1.3s" : "none";
  ...
  if (Number.isFinite(lvl) && lvl >= 11 && lvl <= 249) {
    return (
      <img
        src={bgSkyImg}
        alt="배경"
  ...
  if (Number.isFinite(lvl) && lvl >= 250 && lvl <= 499) {
    return (
      <img
        src={bgMoonImg}
        alt="배경"
  ...
  if (Number.isFinite(lvl) && lvl >= 500 && lvl <= 749) {
    if (bgAuroraImageHeight > 0) {
      const movePx = (lvl - 500) * FLOOR_STEP_PX;
  ...
  if (Number.isFinite(lvl) && lvl >= 750) {
    return (
      <img
        src={bgSpaceImg}
        alt="배경"
